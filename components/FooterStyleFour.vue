<template>
  ...
  >
    <div
      class="bk-footer-inner pt--150 pb--100 pt_sm--80 pb_sm--40 pt_md--80 pb_md--40"
    >
      <div class="row">
        <div class="col-lg-6 col-xl-6">
          <div class="bk-footer-widget">
            <h2 class="heading heading-h2 text-white line-height-1-39">
              Start working on a <br />
              new project?
            </h2>
            <p class="text-white" v-html="footerSecond?.content"></p>
          </div>
        </div>
            ...
            <div class="footer-address">
              <div class="row">
                <div class="col-lg-7 col-md-6 col-sm-6 col-12">
                  ...
                  <div
                    class="social-share social--transparent text-white mt--45"
                  >
                ...
                <div class="col-lg-5 col-md-6 col-sm-6 col-12 mt_mobile--30">
                  <p class="text-white" v-html="footerFirst?.content"></p>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  async mounted() {
    ...
    const filterData = JSON.parse(
      JSON.stringify(data?.filter((sec) => sec?.section === "footer")[0])
    );
    this.footerFirst = filterData?.children?.filter((sec) => sec?.section === "footer-1")[0]
    this.footerSecond = filterData?.children?.filter((sec) => sec?.section === "footer-2")[0]
  },
};
</script>
